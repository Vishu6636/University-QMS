...
def render(uni, user) -> None:
    ...
    for msg in history:
        with st.chat_message(msg["role"]):
            ...
            if msg["role"] == "assistant" and msg.get("escalate"):
                st.warning("⚠️ This query has been flagged for escalation to a support ticket.", icon="🎫")
    ...
    if query:
        ...
        with st.chat_message("assistant"):
            ...
            if result["escalate"]:
                st.warning("⚠️ Flagged for escalation to a support ticket.", icon="🎫")
    ...
    if history:
        ...
        if st.button("🗑️ Clear chat", key=f"clear_chat_{uni.id}_{user.id}", use_container_width=True):
            st.session_state[history_key] = []
            st.rerun()
